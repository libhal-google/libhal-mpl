...
hal::status set_mode(hal::i2c* p_i2c, mpl3115a2::mode p_mode)
{
    ...
    auto ctrl_buffer = HAL_CHECK(hal::write_then_read<1>(
                                        *p_i2c, 
                                        device_address, 
...
*/
hal::status modify_reg_bits(hal::i2c* p_i2c, hal::byte p_reg_addr, hal::byte p_bits_to_set)
{
    // Read old register value
    auto reg_buffer = HAL_CHECK(hal::write_then_read<1>(
                                        *p_i2c, 
                                        device_address, 
...
{   
    ...
    // std::errc error code handler. std::errc::no_such_device_or_address is expected during reset as the device comes online.
...
mpl3115a2::mpl3115a2(hal::i2c& p_i2c) : m_i2c(&p_i2c) 
{}
...
result<mpl3115a2> mpl3115a2::create(hal::i2c& p_i2c)
{
    ...
    // sanity check
    auto whoami_buffer = HAL_CHECK(hal::write_then_read<1>(
                                            p_i2c, 
                                            device_address, 
...
hal::result<mpl3115a2::temperature_read_t> mpl3115a2::read_temperature()
{
    ...
    auto temp_buffer = HAL_CHECK(hal::write_then_read<2>(
                                        *m_i2c, 
                                        device_address, 
...
hal::result<mpl3115a2::pressure_read_t> mpl3115a2::read_pressure()
{
    ...
    auto pres_buffer = HAL_CHECK(hal::write_then_read<3>(
                                        *m_i2c, 
                                        device_address, 
...
hal::result<mpl3115a2::altitude_read_t> mpl3115a2::read_altitude()
{
    ...
    // Read data from out_p_msb_r, out_p_csb_r, and out_p_lsb_r
    auto alt_buffer = HAL_CHECK(hal::write_then_read<3>(
                                        *m_i2c, 
                                        device_address, 
                                        std::array<hal::byte, 1>{ out_p_msb_r }, 
                                        hal::never_timeout()));
